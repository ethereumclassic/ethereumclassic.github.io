# TODOs

```
today
- Roadmap
- tidy existing content
- merge button and link
- pop over warning message for fork announcements etc
- Refine tailwind, DRY component styles, dark theme
- Use tailwind style for icons size
- Netlify Logo
- fix overlay http://localhost:8000/blog/2016-12-29-newsletter/
- replace timeline with proper dates
- blog item sidebar

after
- Generic item sorting/filtering by name, localized
- Videos Section, a bit like news, but more cinema-y, with a collection, move news items to videos?
- Media kit in the extra menu, show logos with color input selector
- Icons tree shaking
- ctrl f TODO
- fix images in german blog posts
- Check https://github.com/gatsbyjs/gatsby/issues/23345, if not fixed split non-blog markdown into it's own renderer with specific imports
- copywriting
- redirect /blog
- Media Kit
- contributors corner
- check all content keys correct
- https://victorzhou.com/blog/minify-svgs/

preflight
- Add comment to all YAML files explaining format __
- Ensure old site feature parity
- i18n 404 redirects, make sure all old pages get redirected
- Update all INTERNAL links (perhaps with whitelist based on the link)
- check page metadata, rss feeds and sitemap are correct
- New Social Sharing Image
- Update Readme
- i18n contributions flow
- Change url refs from netlify

~~~ 3022.V1 RELEASE ~~~

- Github project issues

vNext
- add lastMod last modification date to pages / sitemap for SEO
- use Git Blame to automatically add contributors

Content Ideas
- How to contribute (not just ECIPs)
- Extract concepts from LCETTC
- Story about history and The Original Ethereum vision
- BTC + ETH = ETC
```
