...
Proof of Work is very much like a combustion engine that consumes energy and converts it into useful work in the form of security for the chain, incentivising honest mining and ensuring that 51% attacks are costsly.
...
Over time, the lack of turnover in the staking ecosystem means that a power-block of mega-whales accumulates an ever-increasing poprtion of [all](#race-to-the-bottom) Proof of Stake networks, making these networks increasingly susceptible to takeover, as the nakamoto coefficient approahces 0. This will continue for generations, as big stakers pass their increasingly large bags to the next generation; it is merely a matter of time before the Proof of Stake networks are explicitly captured, let alone covertly.

### Not Permissionless

Permissionlessness, unsurprisingly, is a property that allows new users to onboard into a system without anyone's permission, making the system available to all. Becuase of this, all other things being equal, systems that are permissionless are quantifiably more decentralized than permissioned systems, as they are indiscriminately open to new participants.

Mining, and in turn, Proof of Work Consensus Mechanisms, are permissioness systems, as any anyone can contribute hashrate to the network and have an equal chance of being able to mine a block based on the hashrate they contribute; each hash is like a lottery ticket, and the more tickets purchased the higher the chance of finding a block.

Becuase there are no gatekeepers in the protocol, PoW consensus mechanisms _can't_ care _how_ hashrate is created, or _who_ contributed it. This is an essential quality if a chain is to remain neutral and uncaptured, becuase if gatekeepers existed, they would be able to reject work based on a contributor's identity or location, locking them out, and by doing so, capturing the network.

Proof of Stake systems are _not_ permissionless, as in order to hold the currency reqruired to stake a chain, the only way to get it is by _buying_ it from someone who already has said currency; they need permission from someone inside the network to use the network.

In a sense, all Proof of Stake networks are _already_ soft captured in this way, as if holders of PoS coins decided to stop trading (or selectively stopped trading with users from some particular jurisdiction), new users have no way to make transactions on the network, regardless of the price they were willing to pay.

Take the example of a person from a less developed or sanctioned nation with little to no ability to transact in dollars; without access to the banking system, and thus onboarding exchanges, and with nobody willing to do an OTC deal in cash in his area, they can always get ETC by simply purchasing some hardware to mine it. But this same user is locked out of all Proof of Stake networks, which makes them far less useful for many users, limiting their network value as a whole.
...
In reality, there is no such thing as a free lunch, and after many years of theorizing and real life experimenting with Proof of Stake systems, the results are in; PoS on a fundamental level cannot come close to matching the security guauntees provided by PoW.
